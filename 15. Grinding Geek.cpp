/*
Grinding Geek
MediumAccuracy: 50.0%Submissions: 26K+Points: 4
...
class Solution{
    public:
    int helper(int idx, int n, int total, vector<int> &cost, vector<vector<int>> &dp){
        if(idx == n) return 0;
        if(dp[idx][total] != -1) return dp[idx][total];
        if(total < cost[idx]){
            return dp[idx][total] = helper(idx+1, n, total, cost, dp);
        }
        else{
            int leftAmount = total - cost[idx];
            int refund = cost[idx] * 0.9;
            leftAmount += refund;
            
            int pick = 1 + helper(idx+1, n, leftAmount, cost, dp);
            int notPick = helper(idx+1, n, total, cost, dp);
