...
/*
Approach:
1. Initialize a result array `res` of the same size as `arr` and fill it with -1.
2. Traverse the input array `arr`, and if `arr[i]` is not -1, place `arr[i]` at index `arr[i]` in `res`.
