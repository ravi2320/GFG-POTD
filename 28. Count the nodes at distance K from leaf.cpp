...
MediumAccuracy: 34.27%Submissions: 55K+Points: 4
...
{
    public:
    void help(Node*root, int k, vector<Node*> &vec, unordered_set<Node*> &st, int level){
        if(!root) return;
        ...
        vec.push_back(root);
        help(root->left, k, vec, st, level+1);
        help(root->right, k, vec, st, level+1);
        vec.pop_back();
    }
