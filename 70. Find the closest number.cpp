...
EasyAccuracy: 27.1%Submissions: 69K+Points: 2
...
// Intuition:
// - We need to find the element in the array that is closest to the given value k.
// - We iterate through the array and maintain the index of the element that has the smallest absolute difference with k.
// - If two elements have the same difference, we choose the one that appears later in the array as per the problem requirements.

// Time Complexity: O(N), where N is the number of elements in the array.
// - We iterate through the array once to find the closest element.

// Space Complexity: O(1).
// - We use a few extra variables to track the index and differences, making the space complexity constant.
...
public:
    int findClosest(int n, int k, int arr[]) { 
        int idx = 0;
        for(int i = 1; i < n; i++) {
            if(abs(arr[i] - k) < abs(arr[idx] - k) || 
               (abs(arr[i] - k) == abs(arr[idx] - k) && i > idx)) {
                idx = i;
            }
        }
        return arr[idx];
